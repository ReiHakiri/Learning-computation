...
Inductive Form_term: Type :=
  ...
  | write: Form_term
  | remove: Form_term
  | read_1: nat -> nat -> Form_term
  | read_2: nat -> nat -> Form_term
  | read_3: nat -> nat -> Form_term
  | read_4: nat -> nat -> Form_term
  | compare: Form_term
...
Fixpoint Form_expr_eq (F1 F2: Form_term): bool :=
  let fix Form_list_eq (l1 l2: list Form_term): bool :=
    match l1, l2 with
    | [], [] => true
    | F1 :: l1', F2 :: l2' => andb (Form_expr_eq F1 F2) (Form_list_eq l1' l2')
    | _, _ => false
    end
  in
  ...
  | write, write => true
  | remove, remove => true
  | read_1 n m, read_1 n' m' => andb (n =? n') (m =? m')
  | read_2 n m, read_2 n' m' => andb (n =? n') (m =? m')
  | read_3 n m, read_3 n' m' => andb (n =? n') (m =? m')
  | read_4 n m, read_4 n' m' => andb (n =? n') (m =? m')
...
Definition Form_step (program: Form_program): Form_program :=
  ...
  if length(see_line) <=? m then Form_error program else
  ...
  let n := length(files program) - 1 in
  ...
  {| files := files program;
     open_pointer := open_pointer program;
     line_pointer := S(line_pointer program);
     char_1 := char_3 program;
     char_2 := char_4 program;
     char_3 := char_1 program;
     char_4 := char_2 program;
     truth := truth program;
     done := done program;
     error := error program |}
